...
import { checkContactApi, registerApi } from "../services/authService";
...
export default function RegisterModal({ show, onClose, onSwitchToLogin }) {
  ...
  function validate() {
    ...
    if (!contact.trim()) errs.contact = "Vui lòng nhập email hoặc số điện thoại.";
    else if (!emailRegex.test(contact) && !phoneRegex.test(contact)) errs.contact = "Email hoặc số điện thoại không hợp lệ.";

    if (!captchaInput.trim()) errs.captcha = "Vui lòng nhập mã captcha.";
    else if (captchaInput.toLowerCase() !== captchaCode.toLowerCase()) errs.captcha = "Mã captcha không đúng.";

    if (!otp.trim()) errs.otp = "Vui lòng nhập mã OTP.";
    else if (!/^\d{6}$/.test(otp)) errs.otp = "Mã OTP phải gồm đúng 6 chữ số.";
    else if (otp !== simulatedOtp) errs.otp = "Mã OTP không đúng. Vui lòng kiểm tra lại.";

    if (!password) errs.password = "Vui lòng nhập mật khẩu.";
    else if (password.length < 6 || password.length > 32) errs.password = "Mật khẩu phải từ 6 đến 32 ký tự.";
    ...
    else if (name.trim().length < 2) errs.name = "Họ tên phải có ít nhất 2 ký tự.";

    if (!day || !month || !year) errs.dob = "Vui lòng chọn đầy đủ ngày sinh.";
    else if (currentYear - parseInt(year, 10) < 13) errs.dob = "Bạn phải từ 13 tuổi trở lên để đăng ký.";
  ...
  async function handleSendOtp() {
    ...
    try {
      const { exists } = await checkContactApi(contact.trim());
      if (exists) {
        setErrors((e) => ({ ...e, contact: "Email hoặc SĐT này đã được đăng ký. Vui lòng đăng nhập." }));
        return;
      }
    } catch (error) {
      setErrors((e) => ({ ...e, contact: error.message }));
      return;
    }
        ...
        {registerSuccess ? (
          <div style={{ textAlign: "center", padding: "32px 16px" }}>
            ...
            <p style={{ fontSize: 14, color: "#555", lineHeight: 1.7, marginBottom: 8 }}>
              Xin chào <b>{name}</b>! Tài khoản của bạn đã được tạo trên hệ thống.
            </p>
            ...
            <div style={{ marginBottom: "12px" }}>
              ...
              {otpSent && simulatedOtp && (
                <div style={{ marginTop: "6px", padding: "10px 14px", background: "#fff0f3", border: "1px dashed #ff6b81", borderRadius: "6px", fontSize: "13px", color: "#c2185b" }}>
                  ...
                  <div style={{ fontSize: "11px", color: "#888" }}>(Demo, thực tế sẽ gửi qua email/SMS)</div>
                </div>
              )}
            </div>
