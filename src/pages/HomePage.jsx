...
//Hàm sử dụng để thực hiện lấy đường dẫn url của nhãn hàng dựa trên tên thương hiệu
//Nếu không tìm thấy thì đường dẫn sẽ là trang tìm kiếm với từ khóa là tên thương hiệu đó

function shuffleArray(array) { //Hàm để xáo trộn mảng product ngẫu nhiên
  const newArray = [...array]; //Khai báo một mảng với các phần tử của mảng gốc để tránh làm thay đổi dữ liệu
  for (let i = newArray.length - 1; i > 0; i -= 1) {//Duyệt ngược các phần tử từ cuối lên đầu mảng
    const j = Math.floor(Math.random() * (i + 1));//Lấy bất kỳ phần từ nào với chỉ số j
    [newArray[i], newArray[j]] = [newArray[j], newArray[i]];//Hoán đổi vị trí phần tử j với phần tử hiện tại i
  }
  return newArray;//Trả về mảng đã được xáo trộn ngẫu nhiên
}

function parseVietnameseDate(dateString) {//Hàm để chuyển đổi chuỗi thời gian về dạng timestamp để có thể thực hiện sắp xếp
  const [day, month, year] = dateString.split("/").map(Number);//Tách chuỗi ngày tháng năm dựa trên dấu / và chuyển thành mảng Number
  return new Date(year, month - 1, day).getTime();//Trả về đối tượng Date gồm các tham số năm, tháng -1, ngày
}
...
function getOriginalPrice(price, discount = 0) {//Hàm để tính giá gốc của product dựa vào giá hiện tại (giá đã được discount)
  if (!discount || discount <= 0) return null;//Kiểm tra nếu product không có discount hoặc discount <= 0 thì trả về null
  return Math.round(price / (1 - discount / 100));//Ngược lại sẽ trả về giá gốc đã được làm tròn bằng .round
}
function formatTimePart(value) {//Hàm để định dạng thời gian đếm ngược
  return String(value).padStart(2, "0");//Chuyển đổi giá trị thành chuỗi, nếu giá trị có 1 chữ số thì thêm số 0 đằng trước
}
...
const loopImages = [...bannerImages, bannerImages[0]];//Tạo một mảng mới với các phần tử được sao chép từ bannerImages và thêm phần tử tiếp theo là phẩn tử đầu tiên của mảng bannerImages

function HeroBanner({ totalProducts, productCategoryCount, hotVouchers }) {//Component để hiển thị banner
  const [activeSlide, setActiveSlide] = useState(0);
  //Khai báo một useState để quản lý slide đang hiển thị, mặc định ban đầu là slide(0)
  const [isTransitioning, setIsTransitioning] = useState(true);
  //Khai báo useState để quản lý trạng thái chuyển đổi của các slide, mặc định ban đầu là true
  const [showVouchers, setShowVouchers] = useState(false);
  //Khai báo useState để quản lý trạng thái hiển thị của danh sách các voucher
  useEffect(() => {//Sử dụng useEffect để thiết lập một interval(vòng lặp sự kiện) để thiết lập trạng thái tự chuyển động của các slide
    const interval = setInterval(() => {
      setActiveSlide((prev) => {
        if (prev >= bannerImages.length) {//Kiểm tra nếu slide hiện tại đã vượt qua số lượng slide
          return bannerImages.length; //Thì trả về số lượng slide để hiển thị slide cuối cùng
        }
        return prev + 1;//Ngược lại thì trả về slide tiếp theo
      });
    }, 4200);//Thời gian chuyển động là 4200ms

    return () => clearInterval(interval);//Kết thúc sẽ dùng clearInterval để xóa interval đã thiết lập để tránh tình trạng rò rỉ, và tốn tài nguyên
  }, []);

  const handleTransitionEnd = () => {//Hàm để xử lý khi kết thúc quá trình chuyển đổi slide
    if (activeSlide >= bannerImages.length) {//Nếu slide hiện tại đã vượt quá số lượng slide
      setIsTransitioning(false);//Cập nhật trạng thái chuyển động là false
      setActiveSlide(0);//Cập nhật slide hiện tại và slide(0)
    }
  };
  useEffect(() => {
    if (!isTransitioning) {
