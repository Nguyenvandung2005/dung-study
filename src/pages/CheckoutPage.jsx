...
function getDeliveryDates() {
  ...
  return {
    fast: { label: `${days[fast.getDay()]}. Trước 10h, ${fast.getDate()}/${months[fast.getMonth()]}`, sub: "NowFree Giao Nhanh 2H", tag: "Trẻ tặng 100k", price: 0 },
    normal: { label: `${days[normal.getDay()]}. ${normal.getDate()}/${months[normal.getMonth()]}`, sub: "Giao trong 48 giờ", price: 0 },
  };
}
...
export default function CheckoutPage() {
  const { cartItems, clearCart, addresses, addAddress, updateAddress, removeAddress, getDefaultAddress } = useCart();
  ...
  const handleOrder = () => {
    ...
    if (paymentMethod === "cod") {
      // COD → xử lý luôn không cần modal
      setLoading(true);
      setTimeout(() => {
        setLoading(false);
      ...
      // Bank / MoMo / VNPay → mở PaymentModal
  ...
  return (
    <div style={{ background: "#f5f5f5", minHeight: "100vh", paddingTop: 24, paddingBottom: 60 }}>
          ...
          <div className="col-lg-8">
            ...
            <div style={card}>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                {/* Giao nhanh */}
                <div onClick={() => setDeliveryOption("fast")}
                  style={{ flex: 1, border: `1.5px solid ${deliveryOption === "fast" ? "#ff6b81" : "#ddd"}`, borderRadius: 10, padding: "12px 14px", cursor: "pointer", background: deliveryOption === "fast" ? "#fff0f3" : "#fff" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#555", paddingLeft: 20 }}>
                    {delivery.fast.sub}{" "}
                    <span style={{ background: "#ff6b35", color: "#fff", fontSize: 10, fontWeight: 700, padding: "1px 6px", borderRadius: 4 }}>{delivery.fast.tag}</span>
                  </div>
                ...
                {/* Giao thường */}
                <div onClick={() => setDeliveryOption("normal")}
                  style={{ flex: 1, border: `1.5px solid ${deliveryOption === "normal" ? "#ff6b81" : "#ddd"}`, borderRadius: 10, padding: "12px 14px", cursor: "pointer", background: deliveryOption === "normal" ? "#fff0f3" : "#fff" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <div style={{ ...card, background: "#fff" }}>
              ...
              <div style={{ marginTop: 10, fontSize: 12, color: "#888" }}>
                ...
                <a href="#" style={{ color: "#ff6b81" }}>Chính sách xử lý dữ liệu cá nhân</a> &{" "}
                <a href="#" style={{ color: "#ff6b81" }}>Điều khoản Hasaki</a>
              </div>
              <div style={{ marginTop: 8, background: "#fff8e1", border: "1px solid #ffe082", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#795548", display: "flex", alignItems: "center", gap: 6 }}>
                ⚠️ Cảnh báo lừa đảo: Chúng tôi chỉ nhận chuyển khoản qua tài khoản công ty.
              </div>
            </div>
          </div>
          ...
          <div className="col-lg-4">
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8e8e8", padding: "20px 24px", position: "sticky", top: 80 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <button style={linkBtn()} onClick={() => navigate("/cart")}>Thay đổi</button>
              </div>
                  ...
                  <div key={item.id} style={{ display: "flex", gap: 10, alignItems: "center", paddingBottom: 10, marginBottom: 10, borderBottom: "1px solid #f5f5f5" }}>
                    ...
                    <div style={{ position: "relative", flexShrink: 0 }}>
                      <img
                        ...
                        style={{ width: 52, height: 52, objectFit: "contain", border: "1px solid #eee", borderRadius: 8, background: "#fafafa" }}
                      />
                      <span style={{
                    ...
                    <div style={{ flexShrink: 0, textAlign: "right" }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#ff6b81", whiteSpace: "nowrap" }}>
                        {formatVnd(item.product.price * item.quantity)}
                      </div>
                      {item.quantity > 1 && (
                        <div style={{ fontSize: 11, color: "#aaa" }}>{item.quantity} × {formatVnd(item.product.price)}</div>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <p style={{ fontSize: 12, color: "#888", textAlign: "center", lineHeight: 1.6 }}>
                ...
                <a href="#" style={{ color: "#ff6b81" }}>Chính sách vận chuyển</a>
              </p>
            </div>
          </div>
      ...
      <AddressModal
        ...
        onClose={() => { if (addresses.length === 0) navigate("/cart"); else setShowAddressModal(false); }}
